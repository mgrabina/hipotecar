...
const BankDetailPage = () => {
  ...
  const credits = context?.data.credits.filter(credit => credit.Banco.toLowerCase().localeCompare(bank?.toLowerCase() ?? "") === 0)

  if (!bank || !context?.data.credits.length) return null;
  ...
  return (
    <>
      <Head>
        ...
        <meta name='description' content={`Descubra los creditos hipotecarios UVA del ${credits[0].Banco}, compare tasas, montos, duraciones y más.`} />
        <meta property='og:title' content={`Creditos Hipotecarios UVA del ${credits[0].Banco}`} />
        <meta property='og:description' content={`Descubra los creditos hipotecarios UVA del ${credits[0].Banco}, compare tasas, montos, duraciones y más.`} />
        ...
        <meta property='twitter:description' content={`Descubra los creditos hipotecarios UVA del ${credits[0].Banco}, compare tasas, montos, duraciones y más.`} />
      </Head>
          ...
          <div>
            ...
            <Accordion key={index}>
              <AccordionSummary expandIcon={<ArrowDown />}>
                <Typography variant='body1'>
                  <strong>{credit.Nombre} {credit.Tipo} {credit['Sueldo En Banco'] === "TRUE" ? "(Sueldo en banco)" : "" }</strong>
                </Typography>
              </AccordionSummary>
              <AccordionDetails>
